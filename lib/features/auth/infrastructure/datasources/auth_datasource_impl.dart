...
class AuthDatasourceImpl extends AuthDatasource {
  ...
  Future<User> checkAuthStatus(String token) async {
    try {
      final response = await dio.post('/auth/check-status', options: Options(headers: {'Authorization': 'Bearer $token'}));

      final user = UserMapper.userJsonToEntity(response.data);
      return user;
    } on DioException catch (e) {
      if (e.response?.statusCode == 401) throw CustomError(e.response?.data['message'] ?? 'Token incorrecto!');
