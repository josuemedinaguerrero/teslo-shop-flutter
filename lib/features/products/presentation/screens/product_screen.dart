...
class _ProductView extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final productFormState = ref.watch(productFormProvider(product));
    final textStyles = Theme.of(context).textTheme;

    return ListView(
      children: [
        SizedBox(height: 250, width: 600, child: _ImageGallery(images: productFormState.images)),
        const SizedBox(height: 10),
        Center(child: Text(productFormState.title.value, style: textStyles.titleSmall, textAlign: TextAlign.center)),
        const SizedBox(height: 10),
      ...
      child: Column(
        ...
        children: [
          ...
          CustomProductField(
            ...
            errorMessage: productFormState.title.errorMessage,
          ),
          CustomProductField(
            isTopField: true,
            label: 'Slug',
            ...
            errorMessage: productFormState.slug.errorMessage,
          ),
          CustomProductField(
            isBottomField: true,
            label: 'Precio',
            keyboardType: const TextInputType.numberWithOptions(decimal: true),
            initialValue: productFormState.price.value.toString(),
            onChanged: (value) => ref.read(productFormProvider(product).notifier).onPriceChanged(double.tryParse(value) ?? -1),
            errorMessage: productFormState.price.errorMessage,
          ),
          const SizedBox(height: 15),
          const Text('Extras'),
          _SizeSelector(
          ...
          _GenderSelector(
            selectedGender: productFormState.gender,
          ...
          CustomProductField(
            ...
            label: 'Existencias',
            keyboardType: const TextInputType.numberWithOptions(decimal: true),
            initialValue: productFormState.inStock.value.toString(),
            onChanged: (value) => ref.read(productFormProvider(product).notifier).onStockChanged(int.tryParse(value) ?? 0),
            errorMessage: productFormState.inStock.errorMessage,
          ),
          CustomProductField(
            maxLines: 6,
            label: 'Descripción',
          ...
          CustomProductField(
            ...
            keyboardType: TextInputType.multiline,
            initialValue: productFormState.tags,
    ...
    return PageView(
      ...
      children: images.map((e) {
        ...
        return Padding(
          ...
          child: ClipRRect(
            borderRadius: const BorderRadius.all(Radius.circular(20)),
            child: FadeInImage(fit: BoxFit.cover, placeholder: AssetImage('assets/images/no-image.jpg'), image: imageProvider),
          ),
        );
      }).toList(),
    );
